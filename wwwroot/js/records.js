...
function renderRows() {
  ...
  pageEntries.forEach(data => {
    ...
    row.innerHTML = `
      ...
      <td>${data.medium}</td>
      <td>${data.location}</td>
      <td>${data.periodCovered}</td>
      <td>${data.filingSystem}</td>
      <td>${data.accessControl}</td>
      <td>${data.retentionPeriod}</td>
    `;
...
// Load existing records from the database when the page first opens
async function loadRecords() {
  try {
    const res = await fetch('/Records/GetAll');
...
form.addEventListener('submit', async (e) => {
  ...
  const payload = {
    ...
    filingSystem: formData.get('filing'),
    accessControl: formData.get('access'),
  ...
  try {
    ...
    btn.innerHTML = `<span class="material-symbols-outlined">error</span> ${escapeHtml(err.message)}`;
    setTimeout(() => {
      btn.innerHTML = originalContent;
      btn.disabled = false;
    }, 2000);
